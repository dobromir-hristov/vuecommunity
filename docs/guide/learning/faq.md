...
[Passing data from Laravel to Vue](https://medium.com/@m\_ramsden/passing-data-from-laravel-to-vue-98b9d2a4bd23)
...
* Vuex as a global store Popular - Most robust and scalable solution.
...
You can also use the same techniques as described in [How do I pass data to distant component](faq.md#how-do-i-pass-data-to-distant-component).
...
* trigger an event from the child via `$emit` and handling it on the parent. Recommended
...
* **extract a new, smaller component** - Recommended That component will have the said parameter as a prop or data property on it.
...
:::warning The event Bus is considered a bad practice. It is harder to debug, harder to track where events come from, and can cause memory leaks if not handled properly. :::
...
* [Emulate render props in Vuejs](https://medium.com/@titouan.creach\_44544/emulate-render-props-in-vuejs-c14086dc8dfa)
...
Jest is a testing framework, where as Vue Test Utils is a utility library (set of helpers). You use Vue Test Utils in tandem with some testing library, like Jest or Mocha. It's purpose is to make testing Vue components easier, by abstracting away complex and repeatable test preparation code.
